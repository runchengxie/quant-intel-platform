...
本文件只负责 market-intel 的仓库本地质量指标。研究 owner 仓库维护各自的
质量门，market-intel 不再同步或扫描相邻仓库。
"""
...
try:
    import tomllib
except ModuleNotFoundError:  # Python 3.10 仍用 tomli 兜底
...
# Subpackages not yet enrolled in the maintainability ratchet. These were
# promoted into src/ as-is (not refactored), so they stay out of the budget
# until a follow-up cleanup tightens them. Mirror the ty src exclude.
...
DEFAULT_RATCHET_BUDGETS: dict[str, int] = {
    "long_lines_over_100": 106,
    "functions_over_100": 9,
...
def discover_python_files(
    ...
    for root_name in roots:
        ...
        for path in root.rglob("*.py"):
            ...
            if any(relative.startswith(excluded) for excluded in excludes):
                continue
            files.append(path)
    return sorted(files)
