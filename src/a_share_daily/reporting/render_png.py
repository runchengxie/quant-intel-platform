...
def render_png(document: ReportDocument, theme: ReportTheme, output_path: Path) -> Path:
    ...
    axis.text(0.055, 0.92, document.report_type.upper(), color=theme.tokens["accent"], fontsize=9, **common)
    axis.text(0.055, 0.84, document.title, color=theme.tokens["ink"], fontsize=25, fontweight="bold", **common)
    axis.text(0.055, 0.78, document.report_date, color=theme.tokens["muted"], fontsize=10, **common)
    y = 0.68
    for group in document.metrics:
        axis.text(0.055, y, group.title, color=theme.tokens["ink"], fontsize=13, fontweight="bold", **common)
        y -= 0.055
        for metric in group.metrics:
            axis.text(0.075, y, f"{metric.label}  {metric.value}{metric.unit}", color=theme.tokens["ink"], fontsize=11, **common)
            y -= 0.045
        y -= 0.02
    for section in document.sections:
        axis.text(0.055, y, section.title, color=theme.tokens["ink"], fontsize=13, fontweight="bold", **common)
        y -= 0.06
        for position in section.positions:
            axis.text(0.075, y, f"{position.symbol}  {position.name} · {position.status}", color=theme.tokens["ink"], fontsize=10, **common)
            y -= 0.04
