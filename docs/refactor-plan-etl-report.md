...
- `PROJECT_ROOT` 重算：原脚本用 `Path(__file__).resolve().parents[1]` 反推仓库根（在 `project_tools/` 层）。迁入 `src/a_share_analysis/factor_tools/` 后，该写法只能到 `src/a_share_analysis/`，故全部改为 `parents[3]`（factor_tools → a_share_analysis → src → 仓库根）。硬编码绝对路径（如 `/home/richard/data/quant/market-data-platform`）只作为当前机器的历史运行说明，正式配置仍使用环境变量。
