> 迁移说明：本仓库正在迁移到 [quant-intel-platform](https://github.com/runchengxie/quant-intel-platform) 和 [quant-intel-deploy](https://github.com/runchengxie/quant-intel-deploy)。当前迁移尚未完成，生产使用前请确认最新迁移状态。
...
本仓库不依赖 GitHub Actions 跑代码质量检查。本地 pre-push hook 只管理 `market-intel` 根仓：
