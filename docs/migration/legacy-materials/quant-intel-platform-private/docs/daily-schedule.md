...
本文记录 `market-intel` 当前负责的报告、投递、数据前置和恢复任务。研究算法、因子实验、消融和策略生产逻辑由 `research-workspace` owner 维护；本仓只在必要时通过公开 CLI 触发正式 artifact 的恢复。

## 设计原则

1. 报告发送优先消费已经发布并通过日期/hash/schema 门禁的 artifact。
2. `market-intel` 不复制研究模型，也不通过旧 submodule 生成候选池或 AI 排名。
3. DailyWatch20 的策略 owner 是 `strategy-pipeline` / `strategy-app`；本仓 05:20 producer 只是公开 CLI 的部署/恢复桥。
4. 19:00 晚报前完成会影响当日报告的数据刷新；19:00 后只做回填、次日早报和数据质量修复。
5. Gateway 自愈由 Gateway 外部的 systemd timer 执行。
6. freshness 以业务日期和 artifact receipt 为准，不以进程退出码代替。
7. 自动恢复有次数和冷却预算；发送动作 fail closed，已有成功发送但回执未对齐时不二次外发。
...
| 时间 | 入口 | Owner 类型 | 作用 |
| --- | --- | --- | --- |
| 05:20 工作日 | `daily-watch20-producer.timer` → `refresh_daily_watch20.sh` | 运维桥 → research-workspace | 检查 DailyWatch20 输入/minute freshness；必要时补分钟数据并调用 `strategy watchlist20 run/freshness` 发布正式 artifact |
| 06:00 / 06:45 | `local-fetch-cross-market.timer` | market-intel | 刷新跨市场快照；第二次给较晚更新的数据源补一次机会 |
...
| 07:00 工作日 | Hermes `morning_pipeline.sh` | market-intel | 校验并发送 DailyWatch20、D11-H5 与晨报；不运行旧 AI picker |
...
| 周六 08:15 | `a-share-style-factor-weekly-refresh.timer` | research-workspace producer bridge | 调用标准 owner 入口发布周度风格因子；本仓不实现因子计算 |
| 周六 09:00 | Hermes `weekly_recap.sh` | market-intel | 消费已发布周度研究产物并发送周报 |
| 开机后约 5 分钟；之后每 45 分钟 | `market-intel-scheduled-recovery.timer` | market-intel | 核对数据、报告输入、正式策略 artifact 与投递状态，并按 allow-list 做有限恢复 |
...
news heat 是可选 research input。若 exact-date artifact 已存在则消费；缺失时 market-intel 不再启动 `hot-sector-screener` 重建，而是按 owner/strategy-pipeline 的 optional-input 语义继续。
...
关键 systemd timer 使用 `Persistent=true` 时，恢复启动后会补触发最近错过的任务；它不会逐日重放所有历史计划，也不会自动把失败的 oneshot 无限重试。
