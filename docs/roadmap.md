...
- `fail_under` 曾暂设 1。历史全量覆盖率约 15%，因此本轮先把门槛提高到 10，挡住明显回退；不把历史值当作本提交的实测结果。
- 后续随测试补齐（见第 4、8 项）逐步把 `fail_under` 上调至 70，并在提高前用完整离线测试重新采集覆盖率。

### 13. 收敛复杂度规则豁免 ⬜

2026-09-26 在全仓临时启用 `PLR0911`、`PLR0913` 后发现 134 条现存诊断，主要是参数过多。不能直接移除全局忽略，否则会让既有复杂度债务一次性阻塞所有检查。当前保留两条忽略，并加入逐文件、逐规则的 no-growth ratchet，按生产模块逐批收敛；不再配置未启用 preview 时无效果的 `PLR0917`。

同日维护性 ratchet 扫描发现 10 个超过 100 行的函数，高于预算 9。拆分 `weekly_basket_command.py` 的长编排函数后回到 9，并已把维护性及 PLR ratchet 加入 PR 质量工作流。后续只收紧 ratchet 预算。
